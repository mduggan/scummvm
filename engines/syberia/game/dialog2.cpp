...
void Dialog2::load() {
	...
	TeButtonLayout *dialogLockBtn = _gui.buttonLayout("dialogLockButton");
	if (!dialogLockBtn)
		error("Dialog2::load: Couldn't find dialog lock button");

	dialogLockBtn->setVisible(false);
	addChild(dialogLockBtn);

	TeButtonLayout *dialogBtn = _gui.buttonLayout("dialog");
	if (!dialogBtn)
		error("Dialog2::load: Couldn't find dialog button");
